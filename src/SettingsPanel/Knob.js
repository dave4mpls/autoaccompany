//
//  React-Canvas-UIKnob
//  From https://www.npmjs.com/package/react-canvas-uiknob
//  MIT License
//  Had to be fixed to work with React 16 (proptypes)
//
...
var Knob = function (_React$Component) {
  ...
  function Knob(props) {
    ...
    _this.eventToValue = function (e) {
      ...
      //--- all this percent stuff (DW) is to keep circle from going round at the top.
      var pctNow = ((val-_this.props.min)/(_this.props.max-_this.props.min));
      var pctBefore = ((_this.props.value-_this.props.min)/(_this.props.max-_this.props.min));
      if (pctNow >= .8 && pctBefore <= .2) val = _this.props.value;
      if (pctBefore >= .8 && pctNow <= .2) val = _this.props.value;
    ...
    _this.renderCentre = function () {
      if (_this.props.displayInput) {
        return null;  // DW: changed to draw number in canvas which is much more robust
        //return _react2.default.createElement('span', {
        //  style: _this.inputStyle(),       
        //}, _this.props.value ,_react2.default.createElement("p", {}, this.props.displayCustom));
  ...
  _createClass(Knob, [{
    ...
    value: function drawCanvas() {
      ...
      if (this.props.displayInput) {
        try {   // some truly ancient browsers may not be able to draw text on a canvas
            ctx.font = ((this.radius/1.4)>>0) + "px " + this.props.font;
            ctx.fillStyle = this.props.fgColor;
            ctx.textAlign = "center"; ctx.textBaseline = "middle";
            ctx.fillText(""+this.props.value, this.xy, this.xy);
        } catch(e) { }
      }
    }
  }]);
...
// For now I am just commenting out the proptypes.  In my "Copious Spare Time" I can
// fix them to refer to React16's proptype object.
// Knob.propTypes = {
